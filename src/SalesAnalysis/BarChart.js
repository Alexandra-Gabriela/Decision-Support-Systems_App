...
const BarChart = ({ data, description }) => {
  if (!data || Object.keys(data).length === 0) {
    // Verifică dacă datele sunt nule sau nevalide
    return <p>Data not available.</p>;
  }

  const chartData = {
    labels: Object.keys(data), // Categoriile sunt cheile obiectului de date
    datasets: [
      {
        label: "Sales Distribution",
        data: Object.values(data), // Valorile pentru fiecare categorie
